    ...
    def _activities(self):
        def _get_commit(comment):
            ...
            try:
                ...
                logger.debug(
                    "[gitlab] %s: ignoring commit %s, download failed", self.web_url[8:], hexsha
                )
                return None
        ...
        for comment in self._glpy_issue.notes.list(sort="asc", iterator=True):
            ...
            if not commit:
                comment_counter += 1
            activities.append(
                GlActivity(self, comment=comment, comment_number=comment_counter, commit=commit)
            )
...
class GlProject:
    ...
    def search(self, pattern, since):
        ...
        logger.debug(
            "[gitlab] %s: searching for '%s' in issues%s", self.web_url[8:], pattern, additional_msg
        )
